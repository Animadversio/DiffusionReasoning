...
#     python check_memorization_CLI.py --ep_step 5000 --ep_start 1000 --ep_stop 1400000 \
...
# Assuming the experiment names follow the format "XXX-RAVEN10_abstract*"
for expname in "${exproot}"/*RAVEN10_abstract_*202404* ; do
    encoding="--encoding digit"
    if [[ "$expname" == *"onehot"* ]]; then
        encoding="--encoding onehot"
    fi

    python check_memorization_CLI.py --ep_step 5000 --ep_stop 1200000 \
